...
interface SessionsTableProps {
  data: SessionRow[];
  /** When true, renders without border/heading wrapper (for use inside CollapsibleSection). */
  compact?: boolean;
}
...
export function SessionsTable({ data, compact = false }: SessionsTableProps) {
  ...
  if (data.length === 0) {
    if (compact) {
      return <p className="text-sm text-gray-500 dark:text-gray-400">No active sessions</p>;
    }
    return (
