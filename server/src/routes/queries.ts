  ...
  app.get<{ Params: IdParam; Querystring: QueryListQuery }>('/api/queries/:id', async (req, reply) => {
    ...
    const result = await pool.query(
      `SELECT
         ...
         AVG(physical_reads_per_sec) AS physical_reads_per_sec,
         SUM(execution_count_delta * COALESCE(avg_physical_reads, 0)) AS total_physical_reads,
    ...
    const rows = result.rows.map((row) => ({
      ...
      avg_physical_reads: Number(row.avg_physical_reads),
      physical_reads_per_sec: Number(row.physical_reads_per_sec),
      total_physical_reads: Number(row.total_physical_reads),
